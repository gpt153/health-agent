...
3. `src/utils/nutrition_search.py` - Added circuit breaker + retry + local cache fallback
...
```bash
# Stop USDA API (or use invalid API key)
# Make 6 food logging requests
# Circuit should open after 5 failures
# 6th request should fail immediately (fast failure)
# Check metrics: circuit_breaker_state{api="usda_api"} should be 1 (OPEN)
```

#### 2. Test Fallback
```bash
# Stop OpenAI API
# Send photo for vision analysis
# Should fallback to Anthropic automatically
# Check logs for: "[FALLBACK] Trying strategy: anthropic_vision"
```

#### 3. Test Retry
```bash
# Simulate flaky network (intermittent failures)
# Make API requests
# Check logs for: "[RETRY] Attempt 2/3 for..."
...
```bash
# Stop USDA API
# Log "chicken breast" (in local cache)
# Should succeed with cached data
# Check logs for: "Local cache hit for 'chicken breast'"
```
...
pytest tests/ -k resilience --cov=src/resilience --cov-report=html
```
...
1. **Distributed Circuit Breaker**: Share state across multiple bot instances (Redis)
2. **Adaptive Timeouts**: Dynamically adjust based on API performance
3. **Rate Limiting**: Prevent hitting API rate limits proactively
4. **Advanced Caching**: Use Redis for distributed cache
5. **Health Checks**: Proactive API health monitoring
6. **Bulkhead Pattern**: Isolate API calls to prevent resource exhaustion
7. **Alerting**: Integrate with PagerDuty/Slack for circuit breaker alerts
8. **Dashboard**: Grafana dashboard for visualizing metrics
